...
class DKPN(WaveformModel):
    ...
    def extract_cf(self, stream):
        _prpr = PreProc(stream.copy(), **self.default_args)
    ...
    def predict(self, st, overlap=15.0, P_threshold=0.3, S_threshold=0.3,
        ...
        _prpr = PreProc(st.copy(), **self.default_args)
        stream = _prpr.get_stream()
...
class PreProc(object):
    ...
    def work(self):
        ...
        # # --- NORMALIZE  input stream matrix (3C) with the matrix peak
        # MM = MM / (
        #     np.max(np.abs(MM), axis=(0, 1), keepdims=True) + self.eps
        #     )
